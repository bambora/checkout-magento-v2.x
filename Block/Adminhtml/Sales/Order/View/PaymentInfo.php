...
class PaymentInfo extends \Magento\Backend\Block\Template
{
        ...
        try {
            ...
            if ($paymentMethod === CheckoutPayment::METHOD_CODE) {
                ...
                if (isset($checkoutMethod)) {
                    ...
                    if (!empty($transactionId)) {
                        $message = "";
                        $transactionOperations = $checkoutMethod->getTransactionOperations($transactionId, $storeId, $message);
                        if (isset($transactionOperations)) {
                            $result = '<tr><td colspan="4" class="bambora_table_title">' . __("Transaction History") . '</td></tr>';
                            $result .= $this->createCheckoutTransactionOperationsHtml($transactionOperations, $storeId, $checkoutMethod);
                        }
                    }
                }
            }
            ...
            $errorMessage = $ex->getMessage();
            $result = __("An error occurred while fetching the transaction history. Please see the Bambora log file for more information.");
            $this->_bamboraLogger->addCommonError($transactionId,
    ...
    public function createCheckoutTransactionHtml($transaction)
    {
        ...
        if (is_array($transaction->information->ecis) && count($transaction->information->ecis) > 0) {
            $res .= '<tr><td>' . __("ECI") . ':</td>';
            $res .= '<td>' . $this->getLowestECI($transaction->information->ecis) . '</td></tr>';
        }
        if (is_array($transaction->information->exemptions) && count($transaction->information->exemptions) > 0) {
            $res .= '<tr><td>' . __("Exemption") . ':</td>';
            $res .= '<td>' . $this->getDistinctExemptions($transaction->information->exemptions) . '</td></tr>';
        }
     ...
     * @return string
     */
    public function createCheckoutTransactionOperationsHtml($transactionOperations, $storeId, &$checkoutMethod)
    {
        $html = "";
        foreach ($transactionOperations as $operation) {
            ...
            if ($eventInfo['description'] != "") {
                ...
                $html .= '<td>' . $this->formatDate($operation->createddate, \IntlDateFormatter::SHORT, true) . '</td>';
                $html .= '<td colspan="2"><strong>' . $eventInfo['title'] . '</strong></td>';
                $amount = $this->_bamboraHelper->convertPriceFromMinorunits($operation->amount, $operation->currency->minorunits);
                if ($amount > 0) {
                    $html .= '<td>' . $this->_priceHelper->currency($amount, true, false) . '</td>';
                } else {
                    $html .= '<td>-</td>';
                }
                $html .= '</tr>';
                ...
                if ($operation->status != 'approved'){
                    $eventInfoExtra = $this->getEventExtra($operation, $storeId, $checkoutMethod);
                ...
                if (isset($operation->transactionoperations) && count($operation->transactionoperations) > 0) {
                    $html .= $this->createCheckoutTransactionOperationsHtml($operation->transactionoperations, $storeId, $checkoutMethod);
                }
            } else {
                if (isset($operation->transactionoperations) && count($operation->transactionoperations) > 0) {
                    $html .= $this->createCheckoutTransactionOperationsHtml($operation->transactionoperations, $storeId, $checkoutMethod);
    ...
    private function getEventExtra($operation, $storeId, &$checkoutMethod){
        ...
        $merchantLabel = "";
         if (!empty($source)) {
             $message = "";
             $responseCode = $checkoutMethod->getResponseCodeDetails($source, $actionCode, $storeId, $message);
